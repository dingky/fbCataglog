...
$strSQL="select p.*,pref.category_id from jos_vm_product  as p left join jos_vm_product_category_xref as pref 
         ...
         where p.product_publish ='y' and product_full_image!=''";
$rs=$conn->Execute($strSQL);
$path = "../components/com_virtuemart/shop_image/product/";
echo $rs->recordCount();
while(!$rs->EOF){
      $sourceFile = $path . $rs->fields['product_full_image'];
    echo $resizedFile = __DIR__ ."/images/". $rs->fields['product_full_image'];
     $Img2Thumb = new Img2Thumb( $sourceFile, 262, 207, $resizedFile, 0, 255, 255, 255 );
